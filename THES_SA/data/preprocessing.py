"""
Data Preprocessing Module (v2.0)
Handles cleaning, normalization, and transformation of financial and textual data.
Technical features simplified per advisor feedback: RSI, MA(10,50), MACD.
...
class FinancialDataPreprocessor:
    ...
    def __init__(self, config_path: str = "config.yaml"):
        ...
        self.scalers = {}
        logger.info("Initialized Financial Data Preprocessor (v2.0)")
    ...
    def add_technical_features(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Add technical indicators per v2.0 thesis outline:
        - RSI (14-day)
        - Moving Averages (10-day, 50-day)
        - MACD (12/26/9)
        - Realized Volatility (20-day rolling std of log returns)
        - Forward log returns (1-day and 5-day targets)
        ...
        """
        logger.info("Adding technical features (v2.0)...")
        df_features = df.copy()
        ...
        for ticker in df_features['Ticker'].unique():
            ...
            df_features.loc[mask, 'Log_Return'] = np.log(
                ticker_df['Close'] / ticker_df['Close'].shift(1)
            )

            # Moving Averages (10-day and 50-day)
            df_features.loc[mask, 'MA_10'] = ticker_df['Close'].rolling(window=10).mean()
            df_features.loc[mask, 'MA_50'] = ticker_df['Close'].rolling(window=50).mean()
            ...
            df_features.loc[mask, 'MACD'] = macd_line
            df_features.loc[mask, 'MACD_Signal'] = macd_line.ewm(span=9, adjust=False).mean()
            df_features.loc[mask, 'MACD_Histogram'] = (
                df_features.loc[mask, 'MACD'] - df_features.loc[mask, 'MACD_Signal']
            )

            # Realized Volatility (20-day rolling std of log returns)
            df_features.loc[mask, 'Realized_Volatility'] = (
                df_features.loc[mask, 'Log_Return'].rolling(window=20).std()
            )

            # Forward log returns (TARGET VARIABLES)
            df_features.loc[mask, 'Forward_Return_1d'] = (
                np.log(ticker_df['Close'].shift(-1) / ticker_df['Close'])
            )
            df_features.loc[mask, 'Forward_Return_5d'] = (
                np.log(ticker_df['Close'].shift(-5) / ticker_df['Close'])
            )
    ...
    def normalize_data(self, df: pd.DataFrame,
        ...
        """
        Apply Min-Max scaling to numerical features (excluding target columns)
        ...
        if exclude_cols is None:
            exclude_cols = ['Date', 'Ticker', 'Forward_Return_1d', 'Forward_Return_5d']
    ...
    def process_pipeline(self, input_filename: str, output_filename: str):
        ...
        """
        logger.info("=" * 60)
        logger.info("Starting financial data preprocessing pipeline (v2.0)")
        logger.info("=" * 60)
        ...
        # Drop rows with NaN from rolling calculations (but keep forward return NaNs at end)
        feature_cols = ['MA_10', 'MA_50', 'RSI', 'MACD', 'MACD_Signal',
                       'Realized_Volatility', 'Daily_Return', 'Log_Return']
        df_features = df_features.dropna(subset=feature_cols)
        logger.info(f"After feature engineering: {len(df_features)} records")

        # Normalize data (excludes target columns)
...
class TextualDataPreprocessor:
    ...
    def filter_relevant_news(self, df: pd.DataFrame,
        ...
        # Keywords for nuclear energy and energy-transition context (v2.0)
        keywords = [
            'nuclear', 'SMR', 'reactor', 'uranium', 'enrichment',
            'NuScale', 'Centrus', 'Lightbridge', 'NexGen', 'Nano Nuclear',
            'Lithium Americas', 'Cameco', 'Constellation Energy', 'BWX Technologies',
            'small modular', 'nuclear fuel', 'energy transition',
